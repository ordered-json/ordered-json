...
[공통 검증기](../scripts/verify.py)의 입력·기대 결과 로더를 그대로 재사용했습니다. 공식 예제 17개, 저장소 문법 사례 98개, JSONTestSuite 318개로 총 433개입니다. 프로젝트별 새 예제나 새 기대값을 만들지 않았습니다.

이 결과는 **키의 최초 위치 유지·마지막 값으로 덮어쓰기**를 기준으로 실행한 기록입니다. Python의 `OrderedDict` 동작은 이 기준과 같습니다.
...
| --- | --- | --- |
| 키 순서·개수·컨테이너 구조 일치 | 17 / 17 | 3 / 17 |
| 값과 숫자 토큰까지 포함한 출력 트리 일치 | 16 / 17 | 2 / 17 |
| 유효한 JSON으로 출력 완료 | 16 / 17 | 14 / 17 |

Python은 키 순서와 컨테이너 구조에서 모든 공식 예제와 일치했습니다. Erlang의 첫 번째 검사 성공 세 개는 `document-order`, `root-scalar`, `empty-and-null-key-overwrite`입니다. 값 종류만 비교되는 스칼라 사례도 분모에 포함되므로 이 수치를 객체 순서 보존율로 일반화하지 않습니다.
...
모든 사례의 판정과 공식 예제의 실제 출력은 [기계 판독 결과](ojson-comparison.json)에 있습니다. 현재 저장소의 다섯 구현도 같은 433개 사례를 다시 실행하여 모두 일치함을 확인했습니다.
